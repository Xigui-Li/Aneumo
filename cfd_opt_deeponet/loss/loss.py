...
class DataLoss_Bypass_Plus(nn.Module):
    def __init__(self, trunk_net,branch_bc_net,branch_bp_net):
        """
        Initializing DataLoss_Bypass_Plus 

        Args:
            trunk_net (nn.Module): Backbone network module
            branch_bc_net (nn.Module): Branch network module BC
            branch_bp_net (nn.Module): Branch network module BP
    ...
    def forward(self, x, y_true, bc):
        
        
        trunk_pred1,trunk_pred2,trunk_pred3,trunk_pred4 = self.trunk(x) # shape 4*[B,Np,Nm]

        branch_pred = self.branch_bc(bc).unsqueeze(-1) # shape [B,Nbc,1]
