...
This script processes CFD data for training by reading, filtering, and reshaping 
NumPy-based CFD data. It also computes inlet-centered normals and packages 
the results for downstream tasks.
...
class Read_Data(nn.Module):
    """
    This class is a PyTorch module that processes CFD data
    Reads and processes CFD data from NumPy files by computing 
    normal vectors, filtering abnormal data, and preparing 
    input (X_sup) and output (Y_sup) tensors.

    Args:
        cfd_path (str): Base directory path containing NumPy files.
    """
    ...
    def compute_normals(self,points, k=10):
        """
        Compute normal vectors for each point by analyzing a local neighborhood.

        Args:
            points (ndarray): Coordinates of points to process.
            k (int): Number of neighbors for neighborhood-based normal calculation.

        Returns:
            tuple:
                centre_points (ndarray): Mean of all points.
                centre_normals (ndarray): Mean normal vector across all points.
        """
        tree = KDTree(points)
        ...
        for i in range(len(points)):
            ...
            # Select eigenvector with smallest eigenvalue (normal direction)
    ...
    def forward(self,case_id,fsup):
        """
        Load and process CFD data arrays, compute inlet normals, 
        remove outliers, and return packaged tensors.

        Args:
            case_id (int): Case identifier to be processed.
            fsup (float): Fraction of samples to retain for supervision.

        Returns:
            tuple:
                X_sup (torch.Tensor): Input features with SDF concatenated.
                Y_sup (torch.Tensor): Output data (e.g., pressure/velocity).
                X_inlet (torch.Tensor): Inlet point coordinates.
                Simple_inlet (torch.Tensor): Inlet center coordinates + normal.
        """        
        i = case_id 
        ...
        # remove abnormal pressure data from internal data
        mask = (y_internal[:,0]!=-0.5) 
        x_internal = x_internal[mask]
        sdf_internal = sdf_internal[mask]
        y_internal = y_internal[mask]        

        # convert numpy data to tensor
        N = len(x_internal)
        num_sup = int(np.floor(fsup*N))  
        id_sup = np.random.choice(N,num_sup,replace=False)

        x_internal = x_internal[id_sup,:]
        sdf_internal = sdf_internal[id_sup,:]
        y_internal = y_internal[id_sup,:]
...
for flow_rate in ['m=0.001','m=0.0015','m=0.002','m=0.003','m=0.0035','m=0.004']:

    raw_cfd_path = 'data/npydata/'+flow_rate+'/'
    DataLoader = Read_Data(raw_cfd_path)
    processed_cfd_path = 'data/cfd/'+flow_rate+'/'
    ...
    for case_id in range(3990, 4032):
        try:
            ...
            # Simple_inlet_ structure: [center_x, center_y, center_z, nx, ny, nz, flow_rate]
            Simple_inlet_ = np.zeros([1,7])
            Simple_inlet_[0,0:6] = Simple_inlet
            Simple_inlet_[0,6] = Flow_Rate

            np.savez(processed_cfd_path+str(case_id)+'.npz',X_sup=X_sup,Y_sup=Y_sup,X_inlet=X_inlet,Simple_inlet=Simple_inlet_)
