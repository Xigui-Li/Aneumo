"""
DeepONet and Swin Transformer-based CFD Surrogate Model with FSDP Implementation

This module implements a surrogate model for computational fluid dynamics (CFD) 
using a combination of Deep Operator Networks (DeepONet) and Swin Transformer architectures. 
...
from PrepareDataset.build_dataloader import build_dataloaders
...
def main(train_data_loader, val_dataloader, save_folder, local_rank, world_size, train_sampler, args, cfg):
    """
    Main training loop for the DeepONet and Swin Transformer-based CFD surrogate model with FSDP implementation.
    ...
    The function implements:
        ...
        2. Model initialization (trunk, branch, and image networks)
    ...
    branch_img_net = nets["branch_img"].to(device)
    ...
    loss_data = loss.DataLoss_Bypass_Plus(trunk_net, branch_img_net, branch_bc_net, branch_bp_net).to(device)

    # Define optimizer for all network parameters
    optimizer = torch.optim.Adam(
        list(trunk_net.parameters()) +
        list(branch_img_net.parameters()) +
    ...
    judge_branch_img, branch_img_state = checkpoint.load_checkpoint(cfg.train.resume_epoch, branch_img_net, optimizer, save_folder, local_rank)
    ...
    branch_img_net = torch.compile(branch_img_net)
    ...
    for epoch_id in range(cfg.train.resume_epoch, cfg.train.epochs+1):
        ...
        metrics = torch.zeros(N_train, 11, device=device)  # 11 是指标数量
        accum_steps = 4  # 每 4 步更新一次梯度

        # Training phase
        for i, (x_train, y_train, x_img_train, x_in_train) in tqdm(
            ...
            with amp.autocast(dtype=target_dtype):
                ...
                x_img_train = x_img_train.squeeze(0).to(device, non_blocking=True)
                ...
                loss_pre, loss_vel, train_p, train_u, train_v, train_w, train_MNAE_p, train_MNAE_u, train_MNAE_v, train_MNAE_w, train_dp = \
                    loss_data(x_train, y_train, x_img_train, x_in_train)
        ...
        with torch.no_grad():
            with amp.autocast(dtype=target_dtype):
                for batch_id, (X_test, Y_test, X_img_test, X_in_test) in enumerate(val_dataloader):
                    # **Move data to GPU**
                    X_test = X_test.to(device,non_blocking=True)
                    Y_test = Y_test.to(device,non_blocking=True)
                    X_img_test = X_img_test.to(device,non_blocking=True)
                    ...
                    _, _, test_p, test_u, test_v, test_w, test_MNAE_p, test_MNAE_u, test_MNAE_v, test_MNAE_w, test_dp = \
                        loss_data(X_test, Y_test, X_img_test, X_in_test)
        ...
        if epoch_id % 50 == 0 and dist.get_rank() == 0:  
            ...
            checkpoint.save_branch_img_net(epoch_id, branch_img_net, optimizer, save_folder, local_rank)
